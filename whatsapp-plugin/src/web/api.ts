...
class ApiClient {
  ...
  completeCrmTodo = async (id: string): Promise<CrmTodoSnapshot> => {
    ...
    if (!response.ok || !payload.todo) throw new Error(payload.message ?? `Aarin 待办完成失败 (${response.status})`);
    return payload.todo;
  };
  createGoodJobTodo = async (input: { title: string; priority: ConversationFollowUp["priority"]; dueAt: string; related: string; customerId?: string; triggerKey?: string }): Promise<unknown> => {
    ...
    if (!response.ok) throw new Error(((await response.json().catch(() => ({}))) as { message?: string }).message ?? `Aarin 待办创建失败 (${response.status})`);
