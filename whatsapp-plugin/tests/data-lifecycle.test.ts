...
const DATABASE_LIFECYCLE_TEST_TIMEOUT_MS = 45_000;
...
describe("database lifecycle", { timeout: DATABASE_LIFECYCLE_TEST_TIMEOUT_MS }, () => {
  ...
  it("keeps cleanup dry-run read-only on a migrated database", async () => {
    const { database, repository } = await testRuntime();
    await seedDemo(database, repository, { nodeEnv: "test" });
  ...
  it("never initializes an unmigrated database while rejecting cleanup", async () => {
