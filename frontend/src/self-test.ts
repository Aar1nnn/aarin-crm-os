...
assert.equal(productConfig.productName, "Aarin CRM");
assert.match(prototype, /<strong>Aarin<\/strong>[\s\S]*<h1>Aarin<\/h1>/, "登录页必须使用 Aarin 品牌");
assert.match(prototype, /<span>A<\/span><span>R<\/span>[\s\S]*<strong>Aarin CRM<\/strong>/, "侧栏必须使用 Aarin 品牌与 AR 标志");
assert.doesNotMatch(prototype, />GoodJob(?: CRM)?</, "用户可见的主界面不得继续展示 GoodJob 品牌");
