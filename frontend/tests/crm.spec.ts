...
import { openView, viewNavigationButton } from "./navigation";
...
test.describe("Aarin CRM prototype pages", () => {
  let runId: string;
  ...
  test("customer navigation resolves the shared view to one outcome control", async ({ page }) => {
    const openCustomerButton = viewNavigationButton(page, "customers");
    const wonCustomerButton = viewNavigationButton(page, "customers", "won");

    await expect(openCustomerButton).toHaveCount(1);
    await expect(wonCustomerButton).toHaveCount(1);
    await openView(page, "customers");
    await expect(openCustomerButton).toHaveClass(/active/);
    await expect(wonCustomerButton).not.toHaveClass(/active/);
    await expect(page.locator("#customerPageTitle")).toHaveText("未成交客户");

    await openView(page, "customers", "won");
    await expect(openCustomerButton).not.toHaveClass(/active/);
    await expect(wonCustomerButton).toHaveClass(/active/);
    await expect(page.locator("#customerPageTitle")).toHaveText("已成交客户");
  });
  ...
  test("reminders and import export modules perform real actions", async ({ page }) => {
    ...
    const download = await downloadPromise;
    expect(download.suggestedFilename()).toContain("Aarin客户清单");
  ...
  test("executive report exports a presentation-ready file", async ({ page }) => {
    ...
    const download = await downloadPromise;
    expect(download.suggestedFilename()).toContain("Aarin-CRM");
