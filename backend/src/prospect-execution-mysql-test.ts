...
async function main() {
  ...
  try {
    ...
    stage = "serialize cancellation observed before provider request start";
    await resetSourcePositionMemoryForScenario(
      cancelledBeforeRequestStore
    );
    const serializedCancelOwner =
      cancelledBeforeRequestStore.users.find(
        (item) => item.id === owner.id
      );
    const serializedCancelStrategy =
      cancelledBeforeRequestStore.prospectStrategies.find(
        (item) => item.id === finishedStrategy.id
      );
    assert.ok(serializedCancelOwner);
    assert.ok(serializedCancelStrategy);
    const serializedCancelRunResult = await createProspectRun({
      store: cancelledBeforeRequestStore,
      user: publicUser(serializedCancelOwner),
      strategyId: serializedCancelStrategy.id,
      ifMatch: prospectStrategyEtag(serializedCancelStrategy),
      idempotencyKey:
        `execution-mysql-serialized-cancel-${randomUUID()}`,
      body: {
        reason: "验证取消请求等待事务锁时不会回退 checkpoint 时间"
      },
      requestId: "execution-mysql-serialized-cancel-run"
    });
    const serializedCancelRunId = serializedCancelRunResult.run.id;
    const serializedCancelKernel = new ProspectExecutionKernel({
      store: cancelledBeforeRequestStore,
      workerId: "mysql-serialized-cancel-worker",
      allowedRunIds: [serializedCancelRunId],
      claimSecret,
      cursorSecret
    });
    await serializedCancelKernel.start(tick());
    const serializedCancelClaim =
      await serializedCancelKernel.claimNext(tick());
    assert.ok(serializedCancelClaim);
    const cancellationObservedAt = tick();
    const serializedCancelStarted =
      await serializedCancelKernel.beginRequest({
        leaseId: serializedCancelClaim.lease.id,
        claimToken: serializedCancelClaim.claimToken,
        now: tick()
      });
    assert.equal(serializedCancelStarted.ready, true);
    if (!serializedCancelStarted.ready) {
      throw new Error("serialized cancellation request was deferred");
    }
    const requestStartedCheckpoint = structuredClone(
      cancelledBeforeRequestStore.prospectExecutionCheckpoints.find(
        (item) => item.runId === serializedCancelRunId
      )!
    );
    const serializedCancellation =
      await serializedCancelKernel.requestCancel(
        serializedCancelRunId,
        cancellationObservedAt
      );
    assert.equal(serializedCancellation.status, "cancel_requested");
    const cancelledInFlightCheckpoint =
      cancelledBeforeRequestStore.prospectExecutionCheckpoints.find(
        (item) => item.runId === serializedCancelRunId
      );
    assert.equal(
      cancelledInFlightCheckpoint?.updatedAt,
      requestStartedCheckpoint.updatedAt
    );
    assert.equal(
      cancelledInFlightCheckpoint?.completionReason,
      "CANCELLED_BY_USER"
    );
    const serializedCancelProvider =
    ...
    const serializedCancelResponse = await serializedCancelProvider.search(
      serializedCancelStarted.request
    );
    if (serializedCancelResponse.step.kind !== "success") {
      throw new Error("serialized cancellation response must succeed");
    }
    const serializedCancelSettlement =
      await serializedCancelKernel.completePage({
        leaseId: serializedCancelClaim.lease.id,
        claimToken: serializedCancelClaim.claimToken,
        result: serializedCancelResponse.step,
        responseHash: serializedCancelResponse.responseHash,
        now: tick()
      });
    assert.equal(serializedCancelSettlement.accepted, false);
    assert.equal(serializedCancelSettlement.lateCancellation, true);
    assert.equal(serializedCancelSettlement.runStatus, "cancelled");
