...
router.get("/oauth/callback/:connectorCode", oauthCallbackLimiter, async (req, res) => {
  ...
  try {
    ...
    res.status(202).type("html").send("<!doctype html><meta charset=\"utf-8\"><title>授权已接收</title><style>body{font:15px system-ui;margin:48px;color:#17202a}main{max-width:520px;margin:auto}h1{font-size:22px}</style><main><h1>授权已接收</h1><p>正在安全交换凭据，请返回 Aarin CRM 完成账号确认。</p></main>");
  } catch (cause) {
