  ...
  async findActiveWorkspaceToolByRemoteName(remoteName: string, actor: SessionUser, connectorCode: string) {
    ...
    return this.tools.find((tool) => tool.remoteName === remoteName && tool.status === "active"
      && this.connections.some((connection) => connection.id === tool.connectionId && connection.status === "active"
        && connection.connectorId === connectorId && connection.teamId === actor.teamId
        && (connection.scope === "team" || (connection.scope === "personal" && connection.ownerId === actor.id)))) || null;
  }
...
repository.tools.push({
  id: "tool_team_pagination", connectionId: connection.id, teamId: salesA.teamId, remoteName: "calendar.list_events",
  stableAlias: "calendar.team.list", displayName: "Team calendar list", description: "read with pagination",
  inputSchemaJson: JSON.stringify({
    type: "object", additionalProperties: false,
    properties: { pageToken: { type: "string" } }
  }),
  outputSchemaJson: "{}", schemaHash: "schema_team_pagination", riskLevel: 2, status: "active", revision: 1,
  discoveredAt: now, reviewedAt: now, reviewedBy: adminA.id, permissionCode: "calendar.list.events.read",
  reviewJson: JSON.stringify({
    fieldAllowlist: ["pageToken"],
    dataEgressPolicy: {
      allowedClassifications: ["public", "business", "personal"],
      fieldClassifications: { pageToken: "secret" },
      secretFieldsDenied: true
    }
  }),
  createdAt: now, updatedAt: now
});
repository.grants.add(`tool_team_pagination:${salesA.teamId}:calendar.list.events.read`);
const paginationCall = await service.createReadOnlyCall(
  salesA,
  "calendar.team.list",
  { pageToken: "opaque-google-pagination-value" },
  "req_team_pagination"
);
assert.equal(paginationCall?.status, "queued");
...
console.log(JSON.stringify({
  ...
  providerPaginationTokenAllowedByReviewedSchema: true,
