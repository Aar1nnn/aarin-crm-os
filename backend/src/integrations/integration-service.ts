...
function classifyField(field: string): DataClassification {
  ...
  if (/^(next)?pagetoken$/u.test(normalized)) return "business";
...
function egressDecision(tool: ToolSnapshot, input: Record<string, unknown>) {
  ...
  const fields = Object.keys(input).sort().map((field) => {
    ...
    const configuredClassification = String(configured[field] || "") as DataClassification | "";
    const legacyPaginationSecret = inferred === "business"
      && configuredClassification === "secret"
      && /^(next)?pagetoken$/u.test(field.replace(/[^a-z0-9]/giu, "").toLowerCase());
    return {
      field,
      classification: legacyPaginationSecret ? inferred : (configuredClassification || inferred)
    };
  });
...
export class IntegrationControlPlaneService {
  ...
  private async workspaceTool(actor: SessionUser, remoteName: string, connectorCode: "microsoft-365" | "google-workspace", connectorName: string) {
    ...
    const tool = await this.repository.findActiveWorkspaceToolByRemoteName(remoteName, actor, connectorCode);
    if (!tool) error("INTEGRATION_TOOL_NOT_APPROVED", `请先连接并审核本人或当前团队的 ${connectorName} 对应工具`, 404);
    return tool;
  }
