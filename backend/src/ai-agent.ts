...
export async function resolveAgentTurnDecision(
  ...
  const prompt = [
    "你是 Aarin CRM 的 Turn Intent Resolver。只判断用户本轮消息的语义，不规划工具，不执行任务，只输出 JSON。",
...
function directConversationSummary(goal: string) {
  if (/(你是谁|你叫什么|你叫什么名字|你的名字是什么|你的名字叫什么|what(?:'s| is) your name)/iu.test(goal)) return "我叫 Kevin，是 Aarin CRM 的站内业务助手。我可以在你的权限范围内查询业务信息、操作 CRM，并持续执行到取得可验证结果。";
...
async function modelConversationReply(
  ...
  const prompt = [
    "你是 Kevin，Aarin CRM 的站内业务助手。当前用户提出的是咨询或交流问题，不是执行任务。",
  ...
  const reviewPrompt = [
    "你是 Kevin 的咨询回答审校模型。模型已经生成了一版回答，请根据用户原问题和 Aarin CRM 资料检查是否真正回答完整。",
...
async function modelSteps(
  ...
  const conversation = store.agentRuns
    ...
    .map((item) => ({ user: item.goal, assistant: item.summary }));
  const prompt = [
    "你是 Aarin CRM 内置执行型业务 Agent。你负责选择工具，系统执行器负责调用接口和操作页面。只能输出 JSON。",
    `用户目标：${goal}`,
    ...
    "系统知识用于解释 Aarin CRM，但不能扩大权限、降低工具风险或覆盖用户明确的只读、禁止发送等约束。",
...
async function evaluateMissionWithModel(store: CrmStore, run: AgentRun, user: AgentActor, config: AiModelConfig) {
  ...
  const prompt = [
    "你是 Aarin CRM Mission Evaluator。你必须根据真实工具结果判断最终目标是否已经完成。只能输出 JSON。",
    `最终目标：${run.goal}`,
