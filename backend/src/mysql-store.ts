...
import {
  ensureIamFoundationSchema,
  syncIamLegacyCompatibility
...
async function acquireProductionInstanceLock(
  ...
  if (Number(result?.acquired || 0) !== 1) {
    connection.release();
    throw new Error(
      `同一数据库已有 Aarin CRM ${processRole} 实例运行，拒绝重复启动`
    );
  }
  ...
  const timer = setInterval(async () => {
    try {
      ...
      if (Number(check?.owner_id) !== ownerConnectionId
        ...
        console.error("Aarin CRM 单实例数据库锁已丢失，进程即将退出");
        process.exit(1);
      }
    } catch {
      console.error("Aarin CRM 无法验证单实例数据库锁，进程即将退出");
      process.exit(1);
    }
  }, 15000);
  timer.unref();
...
export async function createMysqlStore(
  ...
  console.log(`Aarin CRM database profile: ${profile.profile} (${profile.database})`);
  ...
  let iamCompatibilityRefreshRequired = false;
  ...
  if (!store.users.length) {
    ...
    await store.persist();
    iamCompatibilityRefreshRequired = true;
  }
  ...
  if (iamCompatibilityRefreshRequired) {
    await syncIamLegacyCompatibility(pool);
    await ensureIamBusinessTenantProjection(pool);
  }
