...
function cloneProviderFailure(error: unknown, requiresKey: boolean) {
  const normalized = providerErrorFromUnknown(error, "trade", { requiresKey });
  return new ProviderContractError({
...
async function runLogged<T>(
  ...
  const errorOptions = {
    requiresKey: providerRequiresKey(input.provider, input.catalog)
  };
  ...
  try {
    return await withProviderRequestLogging({
      ...
      try {
        ...
        const normalized = providerErrorFromUnknown(error, input.context.operation, errorOptions);
        recordProviderHttpResultIfEmpty({
    ...
    throw providerErrorFromUnknown(error, input.context.operation, errorOptions);
  }
}
  ...
  return await runLogged(runtimeInput, "trade", fingerprintValue, async (credential) => {
    ...
    if (cacheKey && cachePolicy) {
      ...
      try {
        ...
        const failure = cloneProviderFailure(
          error,
          providerRequiresKey(input.provider, input.catalog)
        );
        if (follower) {
          recordProviderHttpResult({
