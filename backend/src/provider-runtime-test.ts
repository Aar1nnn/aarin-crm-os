...
const wikidataProvider = getProvider("wikidata");
assert.ok(wikidataProvider?.search, "Wikidata provider must support anonymous search");
setProviderHttpTestTransport(async () => new Response("", { status: 403 }));
try {
  await assert.rejects(executeProviderSearch({
    provider: wikidataProvider,
    catalog: catalog(wikidataProvider),
    context: context(wikidataProvider.id),
    credential: { apiKey: "" },
    query,
    onLogs() {}
  }), (error: unknown) =>
    ...
    && error.publicMessage.includes("无需配置连接凭据")
    && !error.publicMessage.includes("授权已失效")
  );
} finally {
...
for (const status of [401, 403] as const) {
  const failingProvider = provider(`keyed_http_${status}`, async () => {
  ...
  await assert.rejects(executeProviderSearch({
    ...
    connection: connection(failingProvider.id),
    query,
    onLogs() {}
  }), (error: unknown) =>
    error instanceof ProviderContractError
...
const rateLimitedProvider = provider("http_429", async () => {
  throw providerHttpStatusError(new Response("", { status: 429 }), "Typed Provider");
});
await expectCode(executeProviderSearch({
  provider: rateLimitedProvider,
  catalog: catalog(rateLimitedProvider),
  context: context(rateLimitedProvider.id),
  credential: { apiKey: "" },
  query,
  onLogs() {}
