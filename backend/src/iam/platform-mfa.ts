...
const PLATFORM_MFA_ISSUER = "Aarin CRM";
...
function otpAuthUrl(email: string, secret: string) {
  const issuer = encodeURIComponent(PLATFORM_MFA_ISSUER);
...
export function createPlatformMfaService(pool: mysql.Pool): PlatformMfaService {
  return {
    ...
    async enroll(actor) {
      ...
      return { secret, otpauthUrl: otpAuthUrl(actor.email, secret), recoveryCodes };
    },
...
export const platformMfaTestHelpers = { protect, reveal, totp, matches, hashRecoveryCode, otpAuthUrl };
