...
import { persistAiConfigTestOutcome } from "./ai-config-persistence.js";
import {
...
function collaborationUser(userId: string) {
  if (userId === "system") return { id: "system", name: "Aarin", avatar: "AR", role: "admin" as const, teamId: "" };
...
app.post("/api/profile/test-email", requireAuth, asyncRoute(async (_req, res) => {
  ...
  try {
    const info = await sendOutboundEmail(user, {
      to: testTo,
      subject: "Aarin CRM SMTP 测试邮件",
      body: `这是一封来自 Aarin CRM 的 SMTP 测试邮件。\n\n账号：${user.email}\n时间：${new Date().toISOString()}`
    });
...
app.post("/api/tools/ai-config/test", requireAuth, asyncRoute(async (req, res) => {
  ...
  const store = getStore();
  const updated = await persistAiConfigTestOutcome(
    store,
    req.user!.id,
    config.id,
  ...
  if (!updated) {
    res.status(404).json({ message: "配置不存在或无权更新测试结果" });
    return;
  }
  res.json({ ok: result.ok, message: result.message, config: publicAiConfig(updated), configs: getAiConfigs(req.user!).map(publicAiConfig) });
}));
...
function scheduleMidnightTodoArchive() {
  const run = async () => {
    ...
    if (archived.length) {
      await store.persist();
      console.log(`Aarin CRM archived ${archived.length} todos into history`);
    }
    schedule();
  };
  const schedule = () => {
...
async function startServer() {
  ...
  try {
    ...
    console.error(`Aarin CRM security validation failed: ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
    return;
  }
  const port = Number(process.env.PORT || 4188);
  if (mysqlRequested) {
    try {
      ...
      setStore(store);
      console.log("Aarin CRM using MySQL persistence");
    } catch (error) {
      console.error(`Aarin CRM MySQL unavailable, startup aborted: ${error instanceof Error ? error.message : String(error)}`);
      process.exit(1);
    }
  }
  if (integrationEnabled) {
    try {
      ...
      console.log("Aarin CRM integration control plane enabled");
    } catch (error) {
      console.error(`Aarin CRM integration startup failed: ${error instanceof Error ? error.message : String(error)}`);
  ...
  try {
    ...
    console.error(`Aarin CRM Agent runner startup failed: ${error instanceof Error ? error.message : String(error)}`);
    await store.close?.();
    process.exit(1);
    return;
  }
  ...
  if (!prospectWorkerService && prospectQueueRequired) {
    console.error(
      "Aarin CRM prospect queue startup failed: "
  ...
  try {
    ...
    console.error(`Aarin CRM prospect worker startup failed: ${error instanceof Error ? error.message : String(error)}`);
    activeAgentBackgroundRunner = null;
    await agentBackgroundRunner.stop();
    await store.close?.();
    process.exit(1);
  ...
  try {
    ...
    await agentBackgroundRunner.stop();
    await store.close?.();
    console.error(`Aarin CRM prospect scheduler startup failed: ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
    return;
  }
  ...
  try {
    ...
    await agentBackgroundRunner.stop();
    await store.close?.();
    console.error(`Aarin CRM super search runner startup failed: ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
    return;
  }
  ...
  try {
    ...
    await agentBackgroundRunner.stop();
    await store.close?.();
    console.error(`Aarin CRM outreach sequence runner startup failed: ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
    return;
  }
  ...
  try {
    ...
    await agentBackgroundRunner.stop();
    await store.close?.();
    console.error(`Aarin CRM customer maintenance runner startup failed: ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
    return;
  }
  ...
  try {
    ...
    await agentBackgroundRunner.stop();
    await store.close?.();
    console.error(`Aarin CRM Agent trigger runner startup failed: ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
    return;
  }
  ...
  if (process.env.INBOUND_MAIL_ENABLED === "true") {
    ...
    console.log(`Aarin CRM inbound mail watcher enabled (poll ${Number(process.env.INBOUND_MAIL_POLL_MS || 120_000)}ms, lookback ${Number(process.env.INBOUND_MAIL_LOOKBACK_DAYS || 7)}d)`);
  }
  ...
  if (communicationFrontendDist && existsSync(communicationFrontendDist)) {
    ...
    console.log(`Aarin CRM serving Communication frontend from ${communicationFrontendDist}`);
  }
  if (existsSync(frontendDist)) {
    ...
    console.log(`Aarin CRM serving frontend from ${frontendDist}`);
  }

  const httpServer = app.listen(port, host, () => {
  console.log(`Aarin CRM API listening on http://${host}:${port}`);
  });
  ...
  const shutdown = async (signal: NodeJS.Signals) => {
    ...
    shuttingDown = true;
    console.log(`Aarin CRM received ${signal}, shutting down`);
    try {
      ...
      console.error(`Aarin CRM shutdown failed: ${error instanceof Error ? error.message : String(error)}`);
      process.exit(1);
    }
  };
