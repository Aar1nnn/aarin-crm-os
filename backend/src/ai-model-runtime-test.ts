...
const accessDeniedNetworkError = providerErrorFromUnknown(
  new Error("connect EACCES 203.0.113.10:443"),
  "search"
);
assert.equal(accessDeniedNetworkError.code, "PROVIDER_NETWORK_ERROR");
assert.equal(accessDeniedNetworkError.retryable, true);
assert.equal(accessDeniedNetworkError.publicMessage, "数据源连接异常，可稍后重试");

const staleConfig = config({
  id: "ai_stale",
  lastTestAt: "2026-07-21T00:00:00.000Z",
  lastTestStatus: "failed",
  lastTestMessage: "stale failure"
});
const freshConfig = { ...staleConfig };
const persistenceStore = {
  aiModelConfigs: [staleConfig],
  async persist() {},
  async persistMutation<T>(mutation: () => { value: T; rollback(): void }) {
    this.aiModelConfigs.splice(0, this.aiModelConfigs.length, freshConfig);
    return mutation().value;
  }
} as unknown as CrmStore;
const persistedTestOutcome = await persistAiConfigTestOutcome(
  persistenceStore,
  staleConfig.ownerId,
  staleConfig.id,
  {
...
assert.equal(persistedTestOutcome, freshConfig);
assert.equal(persistenceStore.aiModelConfigs[0]?.lastTestStatus, "passed");
assert.equal(persistenceStore.aiModelConfigs[0]?.lastTestMessage, "DeepSeek 连接测试通过");
...
const deepseekContent = await callAiModel(
  config({
    provider: "deepseek",
  ...
  4_000,
  async (_url, init) => {
    deepseekRequest = JSON.parse(String(init?.body || "{}")) as Record<string, unknown>;
    return new Response(JSON.stringify({
      choices: [{ message: { content: "{\"ok\":true}" } }]
    }), { status: 200, headers: { "content-type": "application/json" } });
  }
);
assert.equal(deepseekContent, "{\"ok\":true}");
