...
const platformSession: SessionUser = {
  ...session(superAdmin),
  iamSource: "platform"
};
const tenantWideAdminSession: SessionUser = {
  ...session(adminA),
  iamSource: "iam",
  iamDataScope: {
    permissionCode: "provider.read",
    tenantWide: true,
    ownerIds: []
  }
};
...
assert.equal(visibleMarketTradeObservations(store, platformSession, "campaign_shared").length, 0);
const tenantWideVisible = visibleMarketTradeObservations(
  store,
  tenantWideAdminSession,
  "campaign_shared"
);
assert.equal(tenantWideVisible.length, 3);
assert.ok(tenantWideVisible.every((item) => item.teamId === "team_a"));
